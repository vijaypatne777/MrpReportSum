sap.ui.define([
	...
	return Controller.extend("PP.mrpReport.controller.Home", {
			...
			if (plant !== "" || mrpArea !== "") {
				...
				if (mrpControl === "") {
					var searchUri =
						...
						// mrpArea +
						// "')";
						// "/A_ProductPlant?%24top=10&%24$orderby=Product&$select=Product,Plant&$filter=( Plant eq '" + plant + "')";
						"/A_ProductPlant?top=10&$orderby=Product&$select=Product,Plant&$filter=( Plant eq '" + plant + "')";
				}
				if (mrpControl !== "") {
					var searchUri =
		...
		onSuggestLive: function (event) {
			...
			var mrpArea = this.getView().byId("idFieldMRPArea").getValue().toUpperCase();
			var h = [];
			// event.mParameters.suggestValue
		...
		onSearch: function (oEvent) {
			...
			if (plant !== "" || mrpArea !== "" || oTokens.length > 0) {
				...
				if (aMaterialStockIt.length !== 0) {
					...
					sap.m.MessageToast.show("No Data Found. Please provide correct data !!!");
				}
			} else {
